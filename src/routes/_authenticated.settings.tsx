...
import { rbacMatrix as matrix, rbacModules as modules } from "@/lib/rbac";
...
function Cell({ v }: { v: "R" | "RW" | "-" }) {
  ...
  return (
    <span className="status-pill border bg-success/15 text-success border-success/30">RW</span>
  );
}
      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Access matrix per role. Your role:{" "}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground border-b border-border">
                  ...
                  {modules.map((m) => (
                    <th key={m} className="pb-3 px-2 font-medium text-center">
                      {m}
                    </th>
                  ))}
                </tr>
        ...
        <div className="card-surface p-5">
          ...
          <dl className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <dd className="mt-1">TransitOps Demo</dd>
            </div>
            <div>
              <dt className="text-xs uppercase tracking-wider text-muted-foreground">Session</dt>
              <dd className="mt-1 text-success">Active</dd>
            </div>
          </dl>
        </div>
      </div>
