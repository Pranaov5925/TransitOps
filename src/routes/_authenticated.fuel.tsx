...
function FuelPage() {
  ...
  const [fuelForm, setFuelForm] = useState({ vehicleId: "", liters: 0, cost: 0, date: new Date().toISOString().slice(0, 10) });
  const [expForm, setExpForm] = useState({ vehicleId: "", kind: "Toll", amount: 0, date: new Date().toISOString().slice(0, 10), note: "" });

  const totalPerVehicle = vehicles.map((v) => {
    const fuelCost = fuel.filter((f) => f.vehicleId === v.id).reduce((a, b) => a + b.cost, 0);
    const maintCost = maintenance.filter((m) => m.vehicleId === v.id).reduce((a, b) => a + b.cost, 0);
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Log fuel, tolls, and other operational costs.</p>
          </div>
          <div className="flex gap-2">
            <button onClick={() => setOpenFuel(true)} className="h-9 px-3 rounded-md bg-primary text-primary-foreground text-sm font-medium">
              + Log Fuel
            </button>
            <button onClick={() => setOpenExp(true)} className="h-9 px-3 rounded-md border border-border text-sm font-medium">
              + Add Expense
            </button>
          </div>
        </div>
          ...
          <div className="card-surface p-5">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {expenses.length === 0 && (
                  <tr><td colSpan={4} className="py-4 text-xs text-muted-foreground italic">No expenses logged.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="card-surface p-5">
          <h2 className="font-display font-semibold mb-3">Operational Cost per Vehicle (Fuel + Maintenance + Other)</h2>
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase tracking-wider text-muted-foreground border-b border-border">
                <th className="pb-2 font-medium">Vehicle</th>
            ...
            <tbody>
              {totalPerVehicle.map(({ v, fuelCost, maintCost, otherExp, total }) => (
                <tr key={v.id} className="border-b border-border/50 last:border-0">
                  <td className="py-2 font-medium">{v.regNumber} · {v.model}</td>
        ...
        <Modal title="Log Fuel" onClose={() => setOpenFuel(false)}>
          <form
            onSubmit={(e) => {
              ...
              addFuel(fuelForm);
              setOpenFuel(false);
              setFuelForm({ vehicleId: "", liters: 0, cost: 0, date: new Date().toISOString().slice(0, 10) });
            }}
            className="space-y-3"
          >
            <label className="block">
              ...
              >
                ...
                {vehicles.map((v) => (<option key={v.id} value={v.id}>{v.regNumber} · {v.model}</option>))}
              </select>
            </label>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Liters" type="number" value={String(fuelForm.liters)} onChange={(v) => setFuelForm({ ...fuelForm, liters: Number(v) })} />
              <Field label="Cost" type="number" value={String(fuelForm.cost)} onChange={(v) => setFuelForm({ ...fuelForm, cost: Number(v) })} />
            </div>
            <Field label="Date" type="date" value={fuelForm.date} onChange={(v) => setFuelForm({ ...fuelForm, date: v })} />
            <button className="w-full h-10 rounded-md bg-primary text-primary-foreground font-semibold">Save</button>
          </form>
        </Modal>
      )}
        ...
        <Modal title="Add Expense" onClose={() => setOpenExp(false)}>
          <form
            onSubmit={(e) => {
              ...
              addExpense(expForm);
              setOpenExp(false);
              setExpForm({ vehicleId: "", kind: "Toll", amount: 0, date: new Date().toISOString().slice(0, 10), note: "" });
            }}
            className="space-y-3"
          >
            <label className="block">
            ...
            <div className="grid grid-cols-2 gap-3">
              <SelectField label="Kind" value={expForm.kind} onChange={(v) => setExpForm({ ...expForm, kind: v })} options={["Toll", "Parking", "Permit", "Cleaning", "Misc"]} />
              <Field label="Amount" type="number" value={String(expForm.amount)} onChange={(v) => setExpForm({ ...expForm, amount: Number(v) })} />
            </div>
            <Field label="Date" type="date" value={expForm.date} onChange={(v) => setExpForm({ ...expForm, date: v })} />
            <Field label="Note" value={expForm.note} onChange={(v) => setExpForm({ ...expForm, note: v })} />
            <button className="w-full h-10 rounded-md bg-primary text-primary-foreground font-semibold">Save</button>
          </form>
        </Modal>
      )}
