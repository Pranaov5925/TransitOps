...
function AnalyticsPage() {
  const { vehicles, trips, fuel, maintenance } = useStore();
  ...
  const avgEff = rows.filter((r) => r.efficiency > 0).reduce((a, b) => a + b.efficiency, 0) / (rows.filter((r) => r.efficiency > 0).length || 1);
  const totalOpCost = rows.reduce((a, b) => a + b.fuelCost + b.maintCost, 0);
  const avgRoi = rows.reduce((a, b) => a + b.roi, 0) / (rows.length || 1);

  const chartData = rows.map((r) => ({ name: r.v.regNumber, fuel: r.fuelCost, maintenance: r.maintCost }));
  const pieData = rows
    .filter((r) => r.distance > 0)
    .map((r) => ({ name: r.v.regNumber, value: r.distance }));

  const colors = ["oklch(0.72 0.17 55)", "oklch(0.7 0.13 230)", "oklch(0.7 0.16 155)", "oklch(0.78 0.15 85)", "oklch(0.62 0.22 25)"];

  const exportCsv = () => {
    const header = ["Vehicle", "Distance km", "Fuel L", "Fuel ₹", "Maint ₹", "Efficiency km/L", "ROI %"];
    const csv = [header.join(",")]
      .concat(
        rows.map((r) =>
          [r.v.regNumber, r.distance, r.totalLiters, r.fuelCost, r.maintCost, r.efficiency.toFixed(2), (r.roi * 100).toFixed(1)].join(","),
        ),
      )
      .join("\n");
      ...
      <div className="space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Fuel efficiency, utilization, costs, and ROI.</p>
          </div>
          <button onClick={exportCsv} className="inline-flex items-center gap-1.5 h-9 px-3 rounded-md bg-primary text-primary-foreground text-sm font-medium">
          ...
          <div className="card-surface p-5">
            ...
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={chartData}>
                ...
                <Legend />
                <Bar dataKey="fuel" fill="oklch(0.72 0.17 55)" name="Fuel ₹" radius={[4, 4, 0, 0]} />
                <Bar dataKey="maintenance" fill="oklch(0.7 0.13 230)" name="Maintenance ₹" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.v.id} className="border-b border-border/50 last:border-0">
                    ...
                    <td className="py-2 text-muted-foreground">₹{(r.fuelCost + r.maintCost).toLocaleString()}</td>
                    <td className="py-2 text-muted-foreground">₹{r.revenue.toLocaleString()}</td>
                    <td className={`py-2 font-medium ${r.roi >= 0 ? "text-success" : "text-destructive"}`}>{(r.roi * 100).toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-[11px] text-muted-foreground mt-2">* Revenue is estimated at ₹40/km for demonstration.</p>
        </div>
      </div>
    </AppShell>
  );
