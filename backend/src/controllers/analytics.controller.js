...
async function vehicleCosts(_req, res) {
  try {
    ...
    const rows = vehicles.map((v) => {
      ...
      return { vehicleId: v.id, regNumber: v.regNumber, model: v.model, fuelCost, maintenanceCost: maintCost, otherExpenses: otherExp, totalOperationalCost: fuelCost + maintCost + otherExp };
    });
...
async function fuelEfficiency(_req, res) {
  try {
    ...
    const [trips] = await db.query("SELECT vehicleId, distanceKm FROM trips WHERE `status` = 'Completed'");

    const rows = vehicles.map((v) => {
      const totalLiters = fuel.filter((f) => f.vehicleId === v.id).reduce((a, b) => a + b.liters, 0);
      const distance = trips.filter((t) => t.vehicleId === v.id).reduce((a, b) => a + b.distanceKm, 0);
      const efficiency = totalLiters > 0 ? distance / totalLiters : 0;
      return { vehicleId: v.id, regNumber: v.regNumber, model: v.model, distanceKm: distance, totalLiters, efficiency: parseFloat(efficiency.toFixed(2)) };
    });
    return res.json(rows);
  } catch (err) {
    console.error(err);
...
async function roi(_req, res) {
  try {
    ...
    const rows = vehicles.map((v) => {
      ...
      return { vehicleId: v.id, regNumber: v.regNumber, model: v.model, revenue, fuelCost, maintenanceCost: maintCost, vehicleCost: v.cost, roi: parseFloat(roiVal.toFixed(4)) };
    });
    return res.json(rows);
  } catch (err) {
    console.error(err);
...
async function exportCsv(_req, res) {
  try {
    ...
    vehicles.forEach((v) => {
      ...
      csvRows.push([v.regNumber, distance, totalLiters, fuelCost, maintCost, efficiency.toFixed(2), (roiVal * 100).toFixed(1)].join(","));
    });
